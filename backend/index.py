...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        for doc in range(len(postings_list)):
            doc_id = postings_list[doc]
            crr_tf = tf_list[doc]
            self.doc_length.setdefault(doc_id, 0)
            self.doc_length[doc_id] += crr_tf
        ...
        self.postings_dict[term] = (
            self.index_file.tell(), len(postings_list), len(encoded_postings_list), len(encoded_tf_list))
